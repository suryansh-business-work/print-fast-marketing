...
export const SITE = {
   ...
   */
  url: SELF_URL,
  /** Stable brand origin — the Organization entity's identity across every site. */
  orgUrl: SITE_URLS.main,
  phone: '800-810-4818',
...
// Navigation.
//
// Every link below is authored as a marketing-site path. `toMainSite` rewrites
// them to absolute URLs when this bundle is built for another origin (the shop
// site), and leaves them untouched on the marketing site itself — so the header
...
export const NAV_LINKS: NavLink[] = RAW_NAV_LINKS.map(toMainSite);

const RAW_FOOTER_SERVICES: NavLink[] = [
...
export const FOOTER_SERVICES: NavLink[] = RAW_FOOTER_SERVICES.map(toMainSite);

const RAW_SERVICES: ServiceCard[] = [
  {
...
export const SERVICES: ServiceCard[] = RAW_SERVICES.map(toMainSite);
